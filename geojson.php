...
/**
 * 5.7以后的mysql 导出数据时，GeomFromText 需改为 ST_GeomFromText
 */
...
function generateSql($data, $out_dir)
{
    ...
    foreach ($arcs as $value) {
        ...
        $POLYGON = "POLYGON((" . $polygon_sub1;
        if ($polygon_sub2) {
            $POLYGON .= "," . $polygon_sub2;
        }
        if ($polygon_sub3) {
            $POLYGON .= "," . $polygon_sub3 . "))";
        }
        $sql = "INSERT INTO `city_geo_gcj02`( `code`,`parent_code`, `deep`, `name`, `ext_path`, `point`, `polygon`) VALUES ($adcode,$parent,$level,'$name','$ext_name',GeomFromText('$point'),GeomFromText('$POLYGON'));";
        file_put_contents($out_dir . "$name.sql", $sql . PHP_EOL, FILE_APPEND);
        file_put_contents($out_dir . "all.sql", $sql . PHP_EOL, FILE_APPEND);
        ...
        if (in_array($name, array("北京", "天津", "上海", "重庆"))) {//,"香港","澳门","台湾
            ...
            $sql = "INSERT INTO `city_geo_gcj02`( `code`,`parent_code`, `deep`, `name`, `ext_path`, `point`, `polygon`) VALUES ($adcode,$parent,2,'$name','$ext_name',GeomFromText('$point'),GeomFromText('$POLYGON'));";
            file_put_contents($out_dir . "$name.sql", $sql . PHP_EOL, FILE_APPEND);
            file_put_contents($out_dir . "all.sql", $sql . PHP_EOL, FILE_APPEND);
        }
    }
...
//        }
        ...
        foreach ($city_arcs as $val) {
            ...
            $city_POLYGON = "POLYGON((" . $city_polygon_sub1;
            if ($city_polygon_sub2) {
                $city_POLYGON .= "," . $city_polygon_sub2;
            }
            if ($city_polygon_sub3) {
                $city_POLYGON .= "," . $city_polygon_sub3 . "))";
            }
            $city_sql = "INSERT INTO `city_geo_gcj02`(`code`, `parent_code`, `deep`, `name`, `ext_path`, `point`, `polygon`) VALUES ($city_adcode,$city_parent,$city_level,'$city_name','$city_ext_name',GeomFromText('$city_point'),GeomFromText('$city_POLYGON'));";
            file_put_contents($out_dir . "$name.sql", $city_sql . PHP_EOL, FILE_APPEND);
            file_put_contents($out_dir . "all.sql", $city_sql . PHP_EOL, FILE_APPEND);
        }
    }
